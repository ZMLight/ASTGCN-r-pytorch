...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True):
    ...
    train_x = file_data['train_x']  # (10181, 307, 3, 12) PEMS04
    train_x = train_x[:, :, 0:1, :]  # (10181, 307, 1, 12) #use all features
    train_target = file_data['train_target']  # (10181, 307, 12)

    val_x = file_data['val_x']  # (3394, 307, 3, 12)
    val_x = val_x[:, :, 0:1, :]  # (3394, 307, 1, 12)
    ...
    mean = file_data['mean'][:, :, 0:1, :]  # (1, 1, 1, 1)
    std = file_data['std'][:, :, 0:1, :]  # (1, 1, 1, 1)

    # ------- train_loader -------
    train_x_tensor = torch.from_numpy(train_x).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T) (Batch, NodeNumber, Feature, TimeSlice)
    train_target_tensor = torch.from_numpy(train_target).type(torch.FloatTensor).to(DEVICE)  # (B, N, T) # Timeslice to predict Tp = 12
